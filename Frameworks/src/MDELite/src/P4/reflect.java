...
public class reflect {
    ...
    public static void main(String[] arg) throws ClassNotFoundException, IOException {
        ...
        for (int i = 0; i < locationClasses.length; i++){
            results = getClassInfo(locationClasses[i], arg[0], i);
            ...
            if (computeYuml){
                ...
                for (int j = 0; j < locationClasses.length; j++){
                    if (superclass.equals(locationClasses[j].getName())){
                        yumlDataSpecs = yumlDataSpecs + "[" + javaTrim(superclass, arg[0]) + "]^-.-[" + javaTrim(locationClasses[i].getName(), arg[0]) + "]\n";
                        break;
                    }
                }
            }
            ...
            for (int i = 0; i < locationClasses.length; i++){
                if (!fieldFriends[i].equals("")){
                    ...
                    for (int j = 0; j < differentTypes; j++){
                        ...
                        yumlDataSpecs = yumlDataSpecs + "[" + javaTrim(locationClasses[i].getName(), arg[0]) + "]<>-" + numInfo[1] + ">[" + numInfo[0] + "]\n";
                    }
                }
            }
            File yumlWriteFile = new File(arg[0] + ".yuml");
        ...
        if (computePl){
            ...
            File plWriteFile = new File("outputs/p4/",arg[0] + ".pl");
        ...
        for (int i = 0; i < fields.length; i++){
            ...
            if (computePl) {
                ...
                if (Modifier.isPublic(modif)){
                    pubOrPri = "public"; }
                plAttribute = plAttribute + "attribute(attribute" + attributeid++ + ",'" + currentName + "','" + currentType + "','" + pubOrPri + "','" + plStatic + "',class" + classid + ").\n";
            }
            if (Modifier.isPublic(modif)){
